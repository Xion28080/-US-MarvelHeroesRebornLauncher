    ...
    public LauncherSettings LoadOrCreate()
    {
        try
        {
            if (File.Exists(_filePath))
            {
                ...
                if (settings != null)
                {
                    if (string.Equals(settings.NewsFeedUrl, "https://play.omeganode.org/launcher/news.json", StringComparison.OrdinalIgnoreCase))
                    {
                        settings.NewsFeedUrl = "https://play.omeganode.org/launcher/news-feed.php";
                        Save(settings);
                    }
